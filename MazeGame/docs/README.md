...
| Folder | Contents |
|--------|-----------|
| **`src/`** | Core game logic, pathfinding, and GUI (modules: grid, load, move, `game_logic`, `pathfinding`, package `gui/`). |
| **`docs/`** | This file — full documentation (install, run, algorithm, structure, examples). |
...
| Public API | `src/game_logic.py` | Re-exports core symbols for `from game_logic import …`. |
...
*Standard library only; GUI and text mode share `game_logic` and `pathfinding`.*
